    ...
    for (i in championListPick) {
        ...
        $("#" + champName + "_pick").click(function(){
            if ($("#" + $(this).attr("id") + "_selected").length == 0) {
                ipcRenderer.invoke("champion-clicked-pick", {championName: $(this).attr("id").split("_")[0], championImageUrl: $(this).children("img").attr("src")});
                ...
                $("#" + $(this).attr("id") + "_selected").click(function(){
                    ipcRenderer.invoke("champion-clicked-pick-remove", {championName: $(this).attr("id").split("_")[0], championImageUrl: $(this).children("img").attr("src")});
                    $(this).remove();
                });
            };
        });
